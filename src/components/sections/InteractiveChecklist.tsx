...
export default function InteractiveChecklist({ categories, t }: InteractiveChecklistProps) {
    ...
    const riskLevel = useMemo(() => {
        ...
        if (percent === 100) return { label: t.risk_labels.none, color: "text-green-400", bg: "bg-green-500/10" };
        if (percent > 80) return { label: t.risk_labels.low, color: "text-blue-400", bg: "bg-blue-500/10" };
        if (percent > 50) return { label: t.risk_labels.medium, color: "text-yellow-400", bg: "bg-yellow-500/10" };
        if (percent > 20) return { label: t.risk_labels.high, color: "text-orange-400", bg: "bg-orange-500/10" };
        return { label: t.risk_labels.critical, color: "text-red-400", bg: "bg-red-500/10" };
    }, [completedCount, totalItems, t.risk_labels]);
                            ...
                            <div key={category.id} className="space-y-4">
                                <div className="flex items-center justify-between border-b border-slate-700/50 pb-2">
                                    ...
                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-tighter">
                                        {catDone} / {category.items.length} {t.completed}
                                    </span>
                                </div>
            ...
            <div className="lg:col-span-4">
                <div className="sticky top-32 p-8 rounded-3xl bg-[#1a1f26] text-white border border-slate-700/50 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="mb-8 relative z-10">
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-2">{t.risk_level}</p>
                    ...
                    <div className="space-y-6 relative z-10">
                        <div>
                            <div className="flex justify-between text-sm font-bold mb-2">
                                <span className="text-green-400">✅ {t.done}: {completedCount}</span>
                                <span className="text-red-400">⚠️ {t.risk}: {remainingCount}</span>
                            </div>
